...
- You need to setup your own Github Account

### Download the Repository as a ZIP

We are going to use this repository as a sample to create your repository from scratch. So please download this repository at https://github.com/mlnotes2718/git_intro as a zip file using the following instructions:
...
### Unzip and Clean the src Folder
...
Your `README.md` should already describe the project. If you want to personalize it, open it and edit the content directly in VS Code.
...
The file `.gitignore` describe any files that you do not want to save in the Github. Typically, we do not upload temporary cache files, disposable temporary folders and files, data files that are beyond Github's limit. 

Another type of files that we do not upload are secret files that contain our passwords. This is especially important for repository that are public.

---

### Install VSCode if you have not done so.


---

### 1.4 Initialize a Git Repository
...
# if you want to change the name of the project now it is the time. Please note that git project is the same as the folder name. Do not use the same folder name 
...
This `.git` folder contains are the git settings, the commit history and the changes

---

Once you're happy with both files, stage and commit them:
...
```bash
git commit -m "initialize project with uv and add README"
```
---
...
Fi you do not have VSCode or it is not possible for your VSCode to connect to Github, you need to create an empty repository on Github first and take note of the link to the repository. Then, you can use the following steps:
...
- Do not initialize it with a README, .gitignore, or license file to avoid conflicts.
- Click Create repository.
- On the next page, copy the remote repository URL (e.g., https://github.com/yourusername/your-repo-name.git).

```bash
# Step 2:
git remote add origin <git-web-address>

# Step 3:
git push -u origin main
```

Once it is pushed, you can see that the content are uploaded to the github website

---

Please copy this note somewhere else and delete the entire folder. Don't worry, all teh file are uploaded to the github website.

in VScode, click on CLone Git repo and download the project into your folder you choose.

notice that the folder name is your github project name

You can also sue terminal in VScode to clone your project using the command below:

`git clone your-github-project-web-address`
...
Use the following command to update your changes

```bash
git add .
git commit -m 'add text file mytext.txt with my notes'
git push
```

You can see from the github web page that a text file is added. The commits show changes made.


Guide:

- Commit often with all related tasks
...
> 💡 **Tip:** Running tests before every commit is a good habit. For simplicity we have omitted this part. If any test fails, fix the code first — don't commit broken work.
...
5. Review the diff, then click **Merge pull request → Confirm merge**. Please note that in real life team coding, usually a team lead or a peer programmer is designated to review the code and merge to the main.
